...
**Yes, this looks like waterfall.** Design before code. Documentation before implementation. That is intentional. The industry moved from waterfall to agile because waterfall's core problem was time — months of design before anyone saw working software, and by then the requirements had changed. Agile solved that by interleaving design and implementation in short cycles. But agile assumed a human was writing the code, and writing was slow. With AI, the bottleneck that made waterfall fail — the gap between "design done" and "working software" — collapses. AI generates code from a finished design in minutes, not months. You get the rigor of waterfall (coherent design, traced decisions, reviewed specs) without the cost (long wait for working software). Each vertical slice goes from design to working code in days, not quarters.

There is also a cost that teams without this discipline pay later: **an incoherent system that needs to be rewritten.** When AI generates code without a shared spec, each session invents its own patterns. The codebase drifts. By the time you have customers depending on it, the system is too fragile to refactor and too inconsistent to extend. Rewriting with customers in production is orders of magnitude harder than designing coherently from the start. This process prevents that — not by slowing down, but by making sure every piece of code traces back to a reviewed design before it ships.
...
|---------|---------|--------------------------|
| **P0** — system down | Fix first. Create the issue retroactively. | Write the LLD for the affected area within 48 hours as a follow-up PR. The post-incident doc is often the BEST documentation — it is grounded in real failure. |
...
| "The AI won't understand our legacy code" | It does not need to understand all of it. It needs the manifest (domain boundaries + facades) and the LLD for the area being changed. The 70% it gets right is useful; the 30% it gets wrong is what the architect corrects in review. |
...
| **Month 6** | The team cannot imagine going back. The documentation IS the system, and the code is generated from it. |

> **The insight that makes brownfield adoption practical:** you do not need the documentation to exist before you start the process. You need an architect and a week. AI does the drafting. The architect does the judgment. The process keeps it current. The documentation builds itself.
