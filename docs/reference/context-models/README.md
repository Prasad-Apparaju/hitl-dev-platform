...
**Claude Code:** Each workflow step — `/hitl:dev-practices`, `/hitl:dev-tdd`, `/hitl:dev-generate-docs`, `/hitl:architect-design-feature`, and so on — is a separate skill rather than one giant instruction block loaded at startup. Skills are lazy-loaded: they enter context only when the user explicitly invokes them. A developer running `/hitl:dev-tdd` pays only for the TDD skill instructions — not for the PM, Ops, or Architect workflows. A 32-step workflow written as a skill costs nothing on sessions where it is never invoked.
