...
1. **Install the plugin** (preferred) or copy skills manually:
   ...
   ```bash
   cp -r ../../ai/claude/ .ai/claude/ai/claude/
   ```
...
7. **Decision packets** go in `docs/decisions/issue-NNN.yaml` — Claude generates one per change before writing code. The template is at `../../ai/shared/templates/decision-packet-template.yaml`
