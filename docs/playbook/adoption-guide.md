...
>
> **Brownfield (existing codebase)?** Continue reading — this guide covers the brownfield onboarding sprint.
...
Run `/hitl:dev-start-brownfield` to onboard the existing codebase. It works through these steps with approval gates at each phase:

1. **Map the codebase** — top-level directory structure, source language and framework
2. **Customize CLAUDE.md** — fill in project-specific conventions from code patterns and architect input
3. **Generate system manifest** — domain boundaries, facade APIs, dependency graph
4. **Architect review of existing system** (`/hitl:architect-review-existing`) — reconstructs real ADRs by interviewing the architect about decisions already baked into the code; documents tech stack, auth approach, error handling, deployment model, and known constraints
5. **Priority component docs** — HLD + LLD for the components most likely to change; generated by `/hitl:dev-generate-docs` for each
6. **Seed registries** — test registry stub from existing test files; incident registry from "what broke in the last 6 months?"
7. **First change issue** — create the first GitHub issue to track the first real change

The skill handles the AI side; the architect handles the judgment — especially at step 4, where the ADRs must reflect actual decisions and not just inferred code patterns.
...
|------|---------|---------------|
| **System manifest** | Scans directory tree + imports → produces domain boundaries, file lists, dependency graph, convention detection | Reviews boundaries ("these two are one domain"), fills in mutation descriptions + preconditions + risk notes |
| **ADRs** (step 4 — `/hitl:architect-review-existing`) | Reads code for framework, auth, error handling, deployment, test patterns; presents findings as interview questions | Confirms or corrects each finding; provides the rationale behind each decision; adds tribal knowledge not visible in code |
| **HLDs + LLDs** (step 5 — `/hitl:dev-generate-docs`) | Reads source files per priority component → extracts interfaces, data shapes, state machines, error patterns | Reviews hot components first; marks others "DRAFT — not reviewed"; corrects any hallucinated behavior |
| **CLAUDE.md + conventions** | Generates from template using observed patterns | Reviews conventions, applies the process to ONE real change as proof-of-concept |
