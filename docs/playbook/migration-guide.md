...
| 3 | Generate tests from the target LLD | `/hitl:tdd` |
| 4 | Human + QA review tests, add edge cases | Manual |
| 5 | AI generates Python code in `V2/` that passes tests | `/hitl:tdd` continues |
| 6 | Code review against the target LLD | Two rounds |
| 7 | Downstream impact brief | `/hitl:impact-brief` |
| 8 | Convention check | `/hitl:check-conventions` |
...
|-----------|-------------|
| Run all convention checks | `/hitl:check-conventions` ([ai/claude/check-conventions/SKILL.md](../../ai/claude/check-conventions/SKILL.md)) or `python tools/check-conventions/runner.py --config convention-checks.yaml --verbose` |
...
Design the target architecture from scratch using `/hitl:generate-docs`:
1. Use `/hitl:generate-docs` to draft HLDs for the target system — AI generates from the design conversation; architect reviews and approves
2. Get architect approval on each HLD before proceeding
3. Use `/hitl:generate-docs` to generate LLDs from the approved HLDs — architect and developer review and correct
...
| Impact analysis | `/hitl:apply-change` ([ai/claude/apply-change/SKILL.md](../../ai/claude/apply-change/SKILL.md)) | Affected components in BOTH current and target system |
| TDD | `/hitl:tdd` ([ai/claude/tdd/SKILL.md](../../ai/claude/tdd/SKILL.md)) | Tests from the target LLD + manifest contracts |
...
| Downstream impact brief | `/hitl:impact-brief` ([ai/claude/impact-brief/SKILL.md](../../ai/claude/impact-brief/SKILL.md)) | 5-section brief from PR diff + manifest + incident registry |
| Convention check | `/hitl:check-conventions` ([ai/claude/check-conventions/SKILL.md](../../ai/claude/check-conventions/SKILL.md)) | Violations caught before CI |
...
| Question | Answer |
|----------|--------|
| "Do we need to document the old system?" | Yes — run `/hitl:generate-docs reverse-engineer`. AI does the drafting; architect reviews. ~1 week. |
...
| "What tools do we use?" | Claude Code + the skills (`/hitl:generate-docs`, `/hitl:tdd`, `/hitl:apply-change`, `/hitl:impact-brief`, `/hitl:check-conventions`) + the convention checker in CI + agentic-platform (if building agents). |
