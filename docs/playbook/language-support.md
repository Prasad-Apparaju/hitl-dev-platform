...
- `/hitl:tdd` — reads an LLD and generates tests, then implementation; works in any language
- `/hitl:architect-design-system` and `/hitl:architect-design-feature` — generate docs, not code
- `/hitl:impact-brief`, `/hitl:qa-plan-tests`, `/hitl:qa-verify-quality` — read YAML and docs
...
1. Extend `/hitl:check-conventions` (in `ai/claude/check-conventions/SKILL.md`) to conditionally run the linter based on the stack declared in `CLAUDE.md` or `system-manifest.yaml`
...
If your project uses multiple languages (e.g., Go backend + TypeScript frontend), apply Tiers 1-2 per language. The process layer already handles mixed stacks — LLDs describe components in whatever language they use, and `/hitl:tdd` generates tests in the appropriate language per component. The only constraint is that each domain in `system-manifest.yaml` should map to source in a single language, so the manifest drift check can apply the right parser per domain.
