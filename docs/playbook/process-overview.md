...
7. 👤🤖 **Test case planning** — QA runs `/qa:plan-tests` to query incident history and contribute regression-required scenarios before the TDD cycle starts; developer lists new tests, updated tests, removed tests, and regression tests from the LLD; each QA-contributed scenario must be acknowledged; record the test plan in the GitHub issue and in `.hitl/current-change.yaml` → `/qa:plan-tests`
