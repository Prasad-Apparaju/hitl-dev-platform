...
|-------|------|-------------|---------|-----|-----------|
| **A** | FP-1 | **Schema + catalog.** Add `crit` / `crit_by_tier` / `no_omit` to every `development` step in `ai/shared/workflows.yaml` (the §3 assignment); extend `ai/shared/templates/change-context.schema.yaml` with `first_pass`, the `skips[]` ledger, and `status` enum values `skipped`/`starter`; define the **shared skip-record schema**. Prove additivity. | — | §2, §3, §4 | CRIT-2, COMPAT-1/2 |
