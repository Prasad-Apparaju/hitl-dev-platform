...
| FR-28 | An **Agentic Design Advisor** encodes agentic-systems expertise as a set of **runnable commands** (`hitl:agentic-*`) that **ask the questions an expert asks**: a thorough intake understands the whole scenario, then HITL **composes a right-sized workflow** of commands for the change (a lens with no data contributes no command). Each command sets a **deterministic floor** — the set of controls #10's per-check activation will enforce (the floor is *derived from* #10 activation, so it can never disagree; Tier scales depth) — while **offering** deferrable rungs, recommends the simplest option that fits (never silently decides), and **authors real manifest fields** so the compound-agentic surface's per-check activation runs the relevant validators. A `hitl:agentic-deploy` command surfaces the **build-vs-buy** decision (managed-by-default), **records** it for a human to carry to the platform/ops track, and provisions nothing. It is the right-sizing **front door** to FR-26 and the agentic instance of the best-practice-advisory (#8). HITL governs the design; the Advisor ships no runtime (requirements: `docs/01-product/agentic-design-advisor/requirements.md`; design: `docs/design/agentic-design-advisor/`) | Should Have | Intake composes a proportionate workflow (depth scales with tier); the floor equals the set of controls #10 activates (verified through #10), and a floor command cannot be dropped except via a recorded waiver #10 reads; every menu decision is recommended + recorded + overridable; the authored manifest passes #10's real validator for the low/high/deep fixtures |
