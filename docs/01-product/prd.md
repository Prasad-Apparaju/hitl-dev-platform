...
| FR-27 | HITL can **generate framework-effectiveness metrics** by deriving them from its own durable artifacts (change files, test/incident/token-cost/readiness registries, git/PR history, gate + waiver records): DORA, governance (gate/waiver/traceability/tier/friction), cost, quality (defect escape/rework/incidents), coherence-over-time, and onboarding time. Output is a machine-readable metrics register + a rendered **local** report; every metric is labeled by evidence class (derived / self-reported / estimated); effectiveness is before/after a baseline captured at onboarding. Emits data + static report only — no live/cross-team dashboard, no BI, no phone-home (EPIC #22; requirements: `docs/01-product/metrics-generation/requirements.md`; design: `docs/design/metrics-generation/`) | Should Have | Derive-metrics skill produces the register + report; PRD §8 'Not yet measured' cells become derived; a captured baseline yields a before/after delta; two-stage Codex validation passes before 2.3.0 ships |
