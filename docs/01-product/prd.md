...
| FR-28 | An **Agentic Design Advisor** encodes agentic-systems expertise as a set of **runnable commands** (`hitl:agentic-*`) that **ask the questions an expert asks**: a thorough intake understands the whole scenario, then HITL **composes a right-sized workflow** of commands for the change (a lens with no data contributes no command). Each command **elicits and recommends** for its lens and **records the decision**; it does **not** author the manifest. HITL recommends a **deterministic floor** (Tier + risk — the controls that shouldn't be skipped) while **offering** deferrable rungs, recommends the simplest option that fits (never silently decides), and produces a **design handoff** (a decision record + a manifest *skeleton* with TODOs). A **human authors** the real manifest in the design phase, and the compound-agentic surface (#10) **validates** it — the Advisor stays in the PM/elicitation lane and never produces design/implementation artifacts. A `hitl:agentic-deploy` command surfaces the **build-vs-buy** decision (managed-by-default), **records** it for a human to carry to the platform/ops track, and provisions nothing. It is the right-sizing **front door** to FR-26 and the agentic instance of the best-practice-advisory (#8). HITL governs the design; the Advisor ships no runtime (requirements: `docs/01-product/agentic-design-advisor/requirements.md`; design: `docs/design/agentic-design-advisor/`) | Should Have | Intake composes a proportionate workflow (recommendation); the recommended floor is recorded and a skip is never silent (owner/reason recorded); every menu decision is recommended + recorded + overridable; the output is a decision record + a manifest skeleton (no authored field values), and a human authors the manifest that #10 then validates — the Advisor authors no design artifact |
