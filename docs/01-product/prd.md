...
| FR-26 | A **compound agentic system** delivery surface governs products that are a graph of deterministic services, simple agents, and deep agents with sync and asynchronous A2A edges: the design classifies each component and names the orchestration pattern, marks the determinism boundary, models A2A/inter-component contracts as manifest `facade_apis`, bounds each agent to necessary-and-sufficient privilege and an approved-tool set (declared, validated, and surfaced as generated posture matrices), and makes every component/edge/segment independently evaluable by PM/QA. HITL governs the design; it ships no runtime, messaging backbone, live dashboard, or eval engine (EPIC #10; requirements: `docs/01-product/compound-agentic-surface/requirements.md`; design: `docs/design/compound-agentic-surface/`) | Should Have | Compound surface selectable in `pm-design-feature`; privilege validator flags over/under-privilege; approved-tool gate blocks unapproved tools; PM can run an independent eval on one segment; two-stage Codex validation (source → built plugin) passes before 2.2.0 ships |
| FR-28 | An **Agentic Design Advisor** encodes agentic-systems expertise as a set of **runnable commands** (`hitl:agentic-*`) that **ask the questions an expert asks**: a thorough intake understands the whole scenario, then HITL **composes a right-sized workflow** of commands for the change (a lens with no data contributes no command). Each command sets a **deterministic floor** (mandatory commands, a function of the existing Tier + agentic risk) while **offering** deferrable rungs (adopt now or defer), recommends the simplest option that fits (never silently decides), and **authors real manifest fields** so the compound-agentic surface's existing per-check activation runs only the relevant validators (no change to it). A `hitl:agentic-deploy` command surfaces the **build-vs-buy** decision (managed-by-default), **records** it for a human to carry to the platform/ops track, and provisions nothing. It is the right-sizing **front door** to FR-26 and the agentic instance of the best-practice-advisory (#8). HITL governs the design; the Advisor ships no runtime (requirements: `docs/01-product/agentic-design-advisor/requirements.md`; design: `docs/design/agentic-design-advisor/`) | Should Have | Intake composes a short workflow for a low-risk change and a full one for high-risk; a floor command cannot be dropped silently (only via a recorded waiver); every menu decision is recommended + recorded + overridable; the authored manifest activates only the relevant FR-26 validators, unchanged |
