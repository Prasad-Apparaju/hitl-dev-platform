...
| **ADV-5** | The **floor is the set of mandatory commands**, computed **deterministically** from the existing HITL **Tier** (0–3) plus agentic risk factors (side effects, data sensitivity, autonomy, scale). Commands **above** the floor are **offered as deferrable rungs** (the ladder). The floor reuses Tier; the ladder adds no competing governance axis — it is the "which commands, mandatory vs offered" dimension on top of Tier. | Must |
...
| **ADV-8** | The commands **produce real manifest fields and design artifacts**. The compound-agentic surface (#10) then activates **only the relevant validators from the manifest content it finds** (its existing per-check activation) — the Advisor **configures by producing the right manifest, it does not feed #10 a validator list**. No change to #10 is required. **Exception (honest):** two obligations have **no #10 target today** — the **kill-switch** (from `agentic-reliability`) and **`agentic-observability`** produce **declared design artifacts** in the decision record (recorded, human-reviewed), *not* manifest fields #10 validates; observability's #10 validator arrives with **#15** (#10's CR-9 is deferred there). Every other command authors manifest fields #10 already reacts to. | Must |
...
| **ADV-12** | The floor is **auditable and non-silently-droppable.** Given the **declared** risk factors, the floor is a deterministic function of Tier + risk (ADV-5), so the floor *computation* is reproducible (the requirement does **not** claim two people describe the same scenario identically — risk-factor elicitation uses categorical definitions, per design, to minimize that variance). A floor command **cannot be dropped silently**; dropping one requires an **explicit, recorded, tier-appropriate waiver** — HITL's own hard-gate precedent (PRD **FR-25**: "hard-blocked until … **or waived**") and consistent with "humans decide" (ADV-9). Below the floor *without* a waiver is a blocker. Above the floor, the team defers freely. | Must |
| **ADV-13** | The **intake selects the delivery surface and composes the compound-agentic workflow** when the elicited system has **≥2 components and ≥1 inter-component edge** (a single-component product stays on the existing single-agent surface). The selection + composition integrate into **`pm-design-feature`** and **`ai/codex/AGENTS.md`**. *(Absorbed from #10 CR-1.)* | Must |
| **ADV-15** | The front door generates an **evolving system map** — a **design-time** view (extends #10's generated topology view; **not** a runtime dashboard) that **regenerates incrementally as the intake proceeds** (a component named/typed, a boundary drawn, a gate added), from the accumulating scenario record — so as agents are added one at a time the map grows one node/annotation at a time. It shows the composed topology and a **getting / available / not-needed** breakdown, so the team can *picture the system* and *see the proportionality* — what is included, what is offered, and what is left out **and why**. It reads with a **consistent visual vocabulary per component type** (a datastore looks like a datastore, an agent like an agent, a message like a message), and on capable surfaces may **update live alongside the discussion** (a combined view). It is **terminal-first — no browser is required** to see it, and it is design-time only (HITL does not run a live server). *(The specific renderings — terminal text, Markdown/Mermaid, optional HTML, the combined live mode, the icon vocabulary, and the no-live-server constraint — are **design**, HLD §9.7 / ADR-A8 / LLD §6.)* | Should |
...
2. A **low-stakes** two-agent read-only flow composes a **short workflow**; the saga, deep-memory,
   reliability, observability, and deployment commands are **not** included.
...
5. The commands **produce manifest fields** that make #10's existing per-check activation run exactly the
   relevant validators (ADV-8) — verified against a low- and a high-risk fixture, with **no change to #10**;
   the kill-switch and observability artifacts are recorded as **declared design artifacts** (no #10 target
   yet), not silently omitted.
