...
|----|-------------|----------|
| **CR-1** | **First Pass is a mode of the determined workflow.** After HITL determines the step plan for a change (as today, tiered), the team may mark individual steps to skip and proceed to build. First Pass is opt-in per step; the unmarked default is the full plan. | Must |
