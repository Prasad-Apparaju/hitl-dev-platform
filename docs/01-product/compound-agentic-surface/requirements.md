...
| **CR-14** | **Declared-capability consistency + least-privilege review** (narrowed from "necessary-and-sufficient" per Codex B4 — HITL cannot derive *actual* necessity without running the code, which is the product's IAM/runtime, not the framework's). Each agent declares its identity, privilege scope, and its **capability sources** — tools, memory access, non-tool `capabilities` (KMS/model-endpoint/delegation/ambient), and edge-invocation scopes. A **validator proves consistency**: every granted scope traces to a declared source (over-privilege = a grant with no source), and every declared use has a grant (under-privilege = a use with no grant). *Whether the declared set is genuinely minimal* is the human reviewer's call at the gate, aided by an **optional read-only runtime drift-check** (declared vs granted-at-runtime). A **privilege-posture artifact** is generated (machine-readable **and** rendered). HITL governs the declaration; it does not run the live IAM plane. | Must |
...
| [#12](https://github.com/Prasad-Apparaju/hitl-dev-platform/issues/12) | `pm-design-feature` + `AGENTS.md` surface branch | CR-1, CR-2, CR-7, CR-11, CR-17, CR-18, CR-19, CR-20 |
...
| [#16](https://github.com/Prasad-Apparaju/hitl-dev-platform/issues/16) | Validators + gates + baseline-eval generator | CR-14, CR-15, CR-8, CR-16, CR-20 |
