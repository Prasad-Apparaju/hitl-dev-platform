...
| `.semgrep/` | Semgrep convention rules (required by `/hitl:check-conventions`) |
| `ci/manifest-drift/` | Manifest drift checker (required by `/hitl:check-conventions`) |
| `tools/scripts/fix_mermaid_br_tags.py` | Mermaid linter (required by `/hitl:check-conventions`) |
