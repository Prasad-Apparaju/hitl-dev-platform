...
| 2 | `python3 -m pytest tools/workflow-catalog/test_derive.py ci/skill-lint/test_check_skills.py ci/manifest-drift/test_check_manifest_drift.py ci/hooks -q` | `93 passed`. Deriver (12), skill-lint (8), drift-checker (4), enforcement hooks (69: intake gate #20 = 14, platform deploy gate + statusline chip #21 = 55 incl. the fail-closed, schema-validation, identity, and canonical-completeness cases from the four independent validation rounds). |
