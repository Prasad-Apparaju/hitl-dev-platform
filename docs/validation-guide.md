...
| 2 | `python3 -m pytest tools/workflow-catalog/test_derive.py ci/skill-lint/test_check_skills.py ci/manifest-drift/test_check_manifest_drift.py ci/hooks -q` | `95 passed`. Deriver (12), skill-lint (8), drift-checker (4), enforcement hooks (71: intake gate #20 = 14, platform deploy gate + statusline chip #21 = 57 incl. the fail-closed, schema-validation, identity, canonical-completeness, and layer-placement cases from the five independent validation rounds). |
