...
This directory contains everything needed to use the HITL AI-Driven Development methodology with [Codex CLI](https://github.com/openai/claude/codex).
...
| `config.toml` | Reference Codex CLI config (`~/.ai/codex/config.toml` or `.ai/codex/config.toml`) |
| `hooks.json` | Codex lifecycle hooks — copy to `.ai/codex/hooks.json` for real-time enforcement |
...
- [Codex CLI](https://github.com/openai/claude/codex) installed and authenticated
...
```bash
bash /path/to/hitl-dev-platform/ai/codex/install.sh /path/to/my-project
```

The installer copies:
- `AGENTS.md` → project root
- `.ai/codex/config.toml` → Codex CLI config (review model and `approval_policy`)
- `.ai/codex/hooks.json` → Codex lifecycle hooks (real-time enforcement)
- `ai/codex/hook-scripts/` → scripts used by both Codex and git hooks
- `.git/hooks/pre-commit` and `.git/hooks/post-commit` → git-level enforcement
- `ai/codex/scripts/hitl-conventions.sh` → convention check runner
- `ci/manifest-drift/`, `scripts/fix_mermaid_br_tags.py`, `.semgrep/` → convention check dependencies
- `ai/shared/templates/hld-template.md`, `ai/shared/templates/lld-component-template.md` → design doc templates

**4. Enable Codex lifecycle hooks**

Open `.ai/codex/config.toml` and confirm:
```toml
[features]
codex_hooks = true
```

This enables `.ai/codex/hooks.json`, which runs HITL context checks before every Write/Edit — the same real-time enforcement as Claude Code's PreToolUse/PostToolUse hooks.
...
- [32-step workflow reference](../../docs/playbook/workflow-reference.md)
