...
**The skip record (First Pass, FR-29).** When intake leaves steps out (Fast Track, or a lightened Full Scale plan), TODO Deferral generalizes to the whole workflow: a deferred design item is recorded in the durable **skip ledger** (`.hitl/current-change.yaml` + `.hitl/skip-ledger.yaml`), with the same stance: *surface the risk, respect the choice*: and it resurfaces at defined boundaries (the follow-up, the next overlapping change, incident review), never mid-build. See `ai/shared/first-pass/language.md`.
