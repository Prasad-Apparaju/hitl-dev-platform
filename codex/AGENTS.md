...
- **Web/Mobile:** A visual reference is required before Phase 5 — do not proceed without one. Ask: "Do you already have a design — Figma export, tool screenshot, or photo of a hand-drawn sketch? If yes, share it and I'll annotate it with the Phase 3 edge cases. If not, let's generate it with Claude Design now." If they share an image: read it, annotate missing states and unhandled edge cases, iterate. If they have nothing: generate with Claude Design — one screen per journey step, states: default, empty, loading, error, success, following existing UI patterns in the codebase. **STOP — iterate until satisfied. Get explicit approval: "Design approved."**
