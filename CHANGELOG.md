...
**Note for existing projects:** `tools/scripts/init-project.sh:158` copies `.semgrep/` at onboarding and nothing re-syncs it afterwards, so a repo onboarded before this release keeps the old rule until `.semgrep/security/sql-injection.yaml` is re-copied by hand.
